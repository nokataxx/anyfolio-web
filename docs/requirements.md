...
| `.txt` (テキスト) | Phase 1（エンコーディング自動検出対応） |
| `.docx` / `.doc` (Word) | Phase 1（アップロード時にテキストへ変換して保存・閲覧） |
...
| `name` | text | ファイル名 |
| `type` | text | `md` / `pdf` / `xlsx` / `pptx`（pptxはPDF変換後のパスを保持） |
...
`.pptx` / `.ppt` ファイルはアップロード時にサーバーサイドでPDFに変換し、変換後のPDFをStorageに保存する。閲覧時は既存のPDFビューアをそのまま利用する。

#### 変換フロー
```
ユーザーが .pptx をアップロード
        ↓ クライアントからSupabase Storageに一時保存
        ↓ Supabase Edge Function をトリガー
        ↓ LibreOffice（headless）でPDFに変換
        ↓ 変換後のPDFをStorageに保存
        ↓ anyfolio_files の storage_path を変換後PDFのパスに更新
        ↓ 一時保存した元ファイルを削除
```
...
| 方式 | 特徴 |
|------|------|
| Supabase Edge Function + LibreOffice Docker | Supabaseインフラ内で完結。Deno Deploy制約あり |
| 外部API（CloudConvert等） | 導入が簡単だがAPIコストが発生 |
| Vercel Serverless Function + LibreOffice Lambda Layer | Vercelデプロイと統合しやすい |

#### 注意事項
- 変換中はローディング表示を行い、完了後に自動で表示を更新する
- 変換エラー時はユーザーに通知し、元ファイルを保持する
- `.ppt`（レガシー形式）もLibreOfficeで変換可能
...
| PowerPoint変換 | サーバーサイドでPDFに変換（LibreOffice / Supabase Edge Function） |
| テキストビューア | encoding-japanese（エンコーディング自動検出） |
| Word変換 | mammoth（docxからテキスト抽出） |
...
| テキスト閲覧 | .txtファイルをエンコーディング自動検出で表示 | 高 |
| Word閲覧 | アップロード時にテキストへ変換し、テキストビューアで表示 | 高 |
