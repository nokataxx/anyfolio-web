...
#### iOS Safari 実機
- [X] `.md` ファイルを開いて Edit ボタン押下で編集モードに入る
- [X] タブで Editor / Preview を切り替えられる
- [X] エディタにテキスト入力できる（フォーカス時に画面がズームしない）
- [X] ソフトキーボード表示時にエディタが隠れない・レイアウトが崩れない
- [X] Save ボタンで保存できる、Saved トーストが出る
- [ ] Cmd/外部キーボード接続時に Ctrl+S で保存できる（任意）
- [X] ノッチ / ホームインジケータ領域にコンテンツが潜り込まない

#### Android（Chrome DevTools デバイスモードでシミュレート確認）
- [X] プリセット端末でレイアウト崩れがないこと
- [X] サイドバーのハンバーガー展開/閉じがタッチで動く
- [X] 編集モード・Editor/Previewタブ切替がタッチで動く
- [ ] 物理 Back ボタンの挙動（Android 実機が無いため未確認・将来課題）
  - 現状 `history.pushState` / `beforeunload` を用いていないため、未保存時に Back されても確認ダイアログは出ない
  - 本アプリの主用途は閲覧で、ネイティブアプリ (`anyfolio-app`) 側が主要な編集導線となる想定のため一旦許容

> iOS Chrome は WebKit 必須(Apple ポリシー)で iOS Safari と同一挙動のため検証省略

#### anyfolio-app (`nokataxx/anyfolio-app`) — 未確認
- [ ] Web 側で編集保存したファイルがアプリ側で最新内容で表示される
- [ ] 変換元が `.txt` / `.docx` / `.doc` だったファイルもアプリ側で正しく MD レンダリングされる
- [ ] 全文検索で編集後の新しい内容がヒットする

#### 任意
- [ ] Lighthouse の Mobile Performance スコア確認(将来最適化課題)
